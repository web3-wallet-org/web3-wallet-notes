...
│                                                           │
│  环境信息 (Environment)                                   │
│  ├─ ADDRESS, BALANCE: 账户信息                            │
│  ├─ CALLER, CALLVALUE: 调用者信息                         │
│  ├─ TIMESTAMP, NUMBER: 区块信息                           │
│  └─ GAS, GASPRICE: Gas信息                                │
│                                                           │
│  日志操作 (Logging)                                       │
│  ├─ LOG0-LOG4: 发射事件                                   │
...
│  MUL, DIV           │ 5                             │
│  SLOAD              │ 100 (热) / 2100 (冷)          │
│  SSTORE             │ 100 (更新) / 20000 (新增)     │
│  MLOAD, MSTORE      │ 3                             │
│  JUMP, JUMPI        │ 8                             │
│  CALL               │ 40 (基础) + 调用成本          │
│  LOG0-LOG4          │ 375 + 数据成本                │
│  SHA3 (KECCAK256)   │ 30 + 6 * 字数                 │
│  CREATE, CREATE2    │ 32000                         │
│  SELFDESTRUCT       │ 5000 (首次) / 0 (后续)        │
└────────────────────────────────────────────────────┘
