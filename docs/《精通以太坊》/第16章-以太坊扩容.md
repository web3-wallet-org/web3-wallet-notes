...
graph TB
    ...
    L1["Layer 1 扩容"]
    DataSharding["数据分片"]
    StateSharding["状态分片 ❌已放弃"]
    
    L2["Layer 2 扩容 主流方案"]
    Rollups["Rollups"]
    Optimistic["Optimistic<br/>Arbitrum<br/>Optimism"]
    ZK["ZK Rollup<br/>zkSync<br/>Starknet<br/>Polygon zkEVM"]
    StateChannels["状态通道<br/>Lightning<br/>Raiden"]
    Sidechains["侧链<br/>Polygon PoS<br/>Gnosis"]
    Plasma["Plasma ❌已淘汰"]
...
```mermaid
graph TD
    A["1️⃣ 用户交易 链下提交"] --> B["2️⃣ 排序器打包 Sequencer"]
    B --> C["3️⃣ 执行交易 生成状态根"]
    C --> D["4️⃣ 提交批次到L1 仅数据不执行"]
    ...
    Challenge["⚠️ 挑战机制"]
    Optimistic["乐观假设 默认有效"]
    FraudProof["欺诈证明 任何人可提交"]
    ...
    Result["✅ 结果"]
    NoChallenge["无挑战 状态最终确认"]
    HasChallenge["有挑战 验证并惩罚"]
...
```mermaid
graph TD
    A["1️⃣ 用户交易 链下提交"] --> B["2️⃣ 排序器打包"]
    B --> C["3️⃣ 执行交易 生成状态变更"]
    C --> D["4️⃣ 生成ZK证明 零知识证明"]
    ...
    E --> L1Submit
    L1Submit --> NewStateRoot
    L1Submit --> ZKProofData
    L1Submit --> TxData
    ...
    Advantages --> InstantFinality
    Advantages --> MathSecurity
    Advantages --> LowCost
...
```mermaid
graph TB
    Traditional["📄 传统交易 CALLDATA"]
    T1["❌ 永久存储在链上"]
    T2["❌ 所有节点必须存储"]
    T3["❌ Gas成本高 ~16 Gas/字节"]
    
    Blob["🎈 Blob交易 EIP-4844"]
    B1["✅ 仅保存18天 ~4096 epochs"]
    B2["✅ 不需所有节点存储"]
    B3["✅ Gas成本大幅降低 ~1 Gas/字节"]
    B4["✅ 每区块最多6个Blob"]
...
```mermaid
graph TB
    Before["⚠️ EIP-4844前 2024-03-13之前"]
    Arb1["Arbitrum $0.5-2"]
    Op1["Optimism $0.5-2"]
    Zk1["zkSync $0.1-0.5"]
    
    After["✅ EIP-4844后 2024-03-13激活"]
    Arb2["Arbitrum $0.01-0.1 ↓95%+"]
    Op2["Optimism $0.01-0.1 ↓95%+"]
    Zk2["zkSync $0.001-0.01 ↓98%+"]
...
graph TB
    ...
    LockMint["🔒 锁定-铸造桥 Lock & Mint"]
    LM1["原理: 源链锁定资产 目标链铸造包装代币"]
    LM2["示例: WBTC renBTC"]
    LM3["⚠️ 风险: 依赖托管人多签"]
    
    BurnMint["🔥 燃烧-铸造桥 Burn & Mint"]
    BM1["原理: 源链燃烧资产 目标链铸造新资产"]
    BM2["示例: Arbitrum Bridge Optimism Bridge"]
    BM3["⚠️ 风险: 需要等待期"]
    
    Liquidity["💧 流动性桥 Liquidity Network"]
    LQ1["原理: 两边流动性池 通过LP中转"]
    LQ2["示例: Hop Protocol Connext"]
    LQ3["⚠️ 风险: 流动性不足 MEV"]
    
    Message["📨 消息传递桥 Message Passing"]
    MS1["原理: 传递通用消息 而非仅资产"]
    MS2["示例: LayerZero Wormhole CCIP"]
    MS3["⚠️ 风险: 验证者安全性"]
    
    BridgeTypes --> LockMint
    LockMint --> LM1
    LockMint --> LM2
    LockMint --> LM3
    
    BridgeTypes --> BurnMint
    BurnMint --> BM1
    BurnMint --> BM2
    BurnMint --> BM3
    
    BridgeTypes --> Liquidity
    Liquidity --> LQ1
    Liquidity --> LQ2
    Liquidity --> LQ3
