...
```mermaid
graph LR
    subgraph Depositor["存款人"]
        A["存入 ETH"] --> B["获得 aETH<br/>生息代币"]
    end
    
    subgraph Borrower["借款人"]
        C["抵押 ETH"] --> D["借出 USDC/DAI<br/>支付利息"]
    end
    
    subgraph Pool["Aave 资金池"]
...
```mermaid
graph TB
    subgraph HighPrice["DAI > $1.00 需求过高"]
        A1["提高稳定费<br/>借贷成本增加"] --> B1["用户减少借贷/偿还DAI"]
        B1 --> C1["DAI供应量减少"]
        C1 --> D1["价格回归$1.00"]
    end
    
    subgraph LowPrice["DAI < $1.00 供应过剩"]
        A2["降低稳定费<br/>提高DSR储蓄利率"] --> B2["用户借入DAI/锁定赚息"]
        B2 --> C2["DAI需求增加"]
        C2 --> D2["价格回归$1.00"]
    end
    
    style HighPrice fill:#e6ffe6
    style LowPrice fill:#ffe6e6
```
