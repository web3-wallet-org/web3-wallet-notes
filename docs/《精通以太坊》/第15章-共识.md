...
├────┼────┼────  ├────┼────┼────  ├────┼────┼────
S0   S1   S2     S32  S33  S34     S64  S65  S66
│    │    │      │    │    │      │    │    │
提议 投票 提议    提议 投票 提议    提议 投票 提议

最终性检查:
- Epoch N 和 N+1 都获得 ≥2/3 投票
- Epoch N 被标记为 "Justified"（已证明）
- Epoch N-1 被标记为 "Finalized"（已最终确认）
```
...
```mermaid
graph LR
    A["Epoch 0<br/>获得 ≥2/3 投票"] -->|Justified| B["Epoch 1<br/>获得 ≥2/3 投票"]
    B -->|Justified| C["Epoch 0<br/>Finalized ✅"]
    C -->|Finalized| D["Epoch 2<br/>Current"]
    
    note["🔒 Finalized 区块无法回滚<br/>即使 51% 攻击也无法改变<br/>除非烧毁 1/3 质押"]
    
    C -.-> note
    
    style A fill:#e3f2fd,stroke:#007bff,color:#333
    style B fill:#e3f2fd,stroke:#007bff,color:#333
...
**奖励明细**：
```
单笔证明奖励（Attestation Reward）：

| 组件                  | 奖励（ETH） | 条件                  |
|----------------------|------------|---------------------|
| 及时性奖励            | ~0.0001    | 在 slot 开始 4秒内投票 |
| 目标正确性奖励        | ~0.0001    | 投票给正确的 epoch 边界 |
| 源正确性奖励          | ~0.0001    | 投票给正确的 justified checkpoint |
| 头区块正确性奖励      | ~0.00005   | 投票给正确的区块       |

单笔提议奖励（Block Proposal Reward）：
- 区块提议奖励: ~0.01 ETH
...
- 离线验证者余额逐渐减少
- 在线验证者获得补偿
- 最终使离线验证者余额低于 16 ETH，自动退出

公式：
penalty_per_epoch = base_reward * inactivity_penalty_multiplier * epochs_since_finality

目标：激励验证者保持在线，恢复网络最终性
```
