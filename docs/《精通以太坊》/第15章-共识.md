...
```
Gasper = Casper FFG（最终性） + LMD GHOST（分叉选择）

┌──────────────────────────────────────────────────────────┐
│                    Casper FFG                             │
│                                                           │
│  目标：实现经济最终性（Economic Finality）                  │
│                                                           │
│  规则：                                                    │
│  1. 验证者必须投票给符合规则的区块                          │
│  2. 不能投票给冲突的区块（Double Vote）                    │
│  3. 不能绕过中间 epoch（Surround Vote）                    │
│                                                           │
│  违规惩罚：                                                │
│  - Slash（罚没）：扣除部分或全部质押                       │
│  - 踢出：从验证者集合移除                                  │
...
Justified?    Justified?    Finalized?    Current
    ↓             ↓             ↓             ↓
    
Epoch 0 获得 ≥2/3 投票
→ Epoch 0 被标记为 Justified

Epoch 1 获得 ≥2/3 投票
→ Epoch 1 被标记为 Justified
→ Epoch 0 被标记为 Finalized（最终确认）

Finalized 区块无法回滚！
即使 51% 攻击也无法改变最终性（除非烧毁 1/3 质押）
```
...
│              以太坊双客户端架构                             │
│                                                           │
│  执行层（Execution Layer）       共识层（Consensus Layer） │
│  ┌──────────────────┐          ┌──────────────────┐      │
│  │  Geth            │          │  Prysm           │      │
