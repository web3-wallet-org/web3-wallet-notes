...
这对合约来说没问题，但对用户和前端都不够友好。

这一章要做的事，就是把这些复杂参数包装成一个更简单的入口：

```text
tokenId
```

以后用户管理仓位时，不需要每次都记住 Pool 和价格范围，而是直接操作自己钱包里的 LP NFT。
...
两个 LP 即使都给 ETH/USDC 提供流动性，<span style="color: red;">只要价格范围不同，它们承担的风险和赚到的费用都不同。</span>
...
Manager 可以通过这个 `tokenId` 找到背后的仓位数据。

第二，所有权和授权更清晰。

谁拥有这个 NFT，谁就拥有这个 LP 仓位。

谁被授权操作这个 NFT，谁就能在规则允许的范围内管理这个仓位。
...
每个仓位可以通过 `tokenURI` 展示自己的图片、属性和描述。

第五，Manager 可以统一管理复杂操作。
...
- ERC721 主要解决 <span style="color: red;">所有权和转让</span> 问题。
