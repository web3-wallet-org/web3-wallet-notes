...
| | 传统交易 CALLDATA | Blob 交易 EIP-4844 |
|---|---|---|
| 存储时间 | ❌ 永久存储在链上 | ✅ 仅保存 18 天（~4096 epochs）|
| 存储要求 | ❌ 所有节点必须存储 | ✅ 不需所有节点存储 |
| Gas 成本 | ❌ ~16 Gas / 字节 | ✅ ~1 Gas / 字节 |
| 容量上限 | — | ✅ 每区块最多 6 个 Blob |
...
```mermaid
graph TB
    Before["⚠️ EIP-4844 前<br/>2024-03-13 之前"]
    Before --> Arb1["Arbitrum<br/>$0.5-2"]
    Before --> Op1["Optimism<br/>$0.5-2"]
    Before --> Zk1["zkSync<br/>$0.1-0.5"]

    Before ==>|"节省 90-99%"| After

    After["✅ EIP-4844 后<br/>2024-03-13 激活"]
    After --> Arb2["Arbitrum<br/>$0.01-0.1 ↓95%+"]
    After --> Op2["Optimism<br/>$0.01-0.1 ↓95%+"]
    After --> Zk2["zkSync<br/>$0.001-0.01 ↓98%+"]

    After --> Impact["💡 影响"]
    Impact --> I1["📈 L2 使用量暴增"]
    Impact --> I2["🏗️ 更多应用迁移 L2"]
    Impact --> I3["💰 主网 Gas 稳定"]
...
```mermaid
graph TD
    A["👤 用户交易"] --> B["1️⃣ 提交到 Polygon 网络"]
    B --> C["2️⃣ 验证者打包区块<br/>~2 秒出块"]
    C --> D["3️⃣ 定期提交检查点到 L1"]
...
| 类型 | 原理 | 代表项目 | 主要风险 |
|------|------|---------|---------|
| 🔒 锁定-铸造桥 | 源链锁定资产，目标链铸造包装代币 | WBTC、renBTC | 依赖托管人多签 |
| 🔥 燃烧-铸造桥 | 源链燃烧资产，目标链铸造新资产 | Arbitrum Bridge、Optimism Bridge | 需要等待期 |
| 💧 流动性桥 | 两端流动性池，通过 LP 中转 | Hop Protocol、Connext | 流动性不足、MEV |
| 📨 消息传递桥 | 传递通用消息，不限于资产转移 | LayerZero、Wormhole、CCIP | 验证者安全性 |
